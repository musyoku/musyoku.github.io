...
次に一つ前のフレームと現在のフレームで最大値を取ります。こうする理由は色々あるのですが、極端に言うと人間の目の残像のような効果を得るためだと思います。
...
とします。これは一般的な二乗誤差関数で、$Q(s,a)$が$target$から離れているほど誤差が大きくなります。

あとはこの誤差関数をChainerで組み立てて誤差逆伝播すればよいのですが、本来定数であるべき教師信号が微分されないように注意する必要があります。

式((3))に含まれる教師信号$$target$$は、式((2))から明らかなようにパラメータ$\theta$を持っています。

したがって誤差をChainerでbackpropすると、教師信号まで含めて微分をしてしまうため、式の意味が変わってきてしまいます。

そこでパラメータ$\theta$をコピーした$\phi$を作り、教師信号は$Q_{\phi}(s,a)$から出力させることで微分を防ぐことができます。
...
Google DeepMindの報告によれば、600エピソードを超えた頃からほとんど取りこぼしがなくなったそうですが、私の実装では4000エピソードかかっています。

おそらくGoogleは--frame_skipを指定せず60fpsで動かしていたと思われますが、詳細は不明です。