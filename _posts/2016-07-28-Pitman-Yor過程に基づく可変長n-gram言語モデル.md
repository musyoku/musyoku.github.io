...
となるので、客$dog$がルートのレストランに追加される確率は$q_{root}$、レストラン$lazy$に追加される確率は$(1-q_{root})q_{lazy}$、レストラン$the$に追加される確率は$(1-q_{root})(1-q_{lazy})q_{the}$、・・・のように計算されます。
...
のようにギブスサンプリングするのですが、この式はベイズの定理から
...
と表すことができます。（$\propto$は比例を意味します）

$\boldsymbol n_{-t}$は、単語$w_t$を除いた単語列$w_1,w_2,w_{t-1},w_{t+1},w_T$に対応するn-gramオーダー$n_1,n_2,...,n_{t-1},n_{t+1},...,n_T$を表しています。

### $n_t$のサンプリング

ここでの$\boldsymbol n_{-t}$はただの整数列であり、この値がそのままモデルのパラメータにあるわけではありません。

そして$n_t$をギブスサンプリングするためには$w_t$以外の単語$w_1,w_2,w_{t-1},w_{t+1},w_T$が文脈木をたどった深さ$n_1,n_2,...,n_{t-1},n_{t+1},...,n_T$をモデル自身が知っている必要があります。
...
$$
	\begin{align}
		P(n_t\mid\boldsymbol w_{-t}, \boldsymbol n_{-t}, \boldsymbol \Theta_{-t})=\frac{a_l+\alpha}{a_l+b_l+\alpha+\beta}\prod_{i=0}^{l-1}\frac{b_i+\beta}{a_i+b_i+\alpha+\beta}
	\end{align}\
$$
