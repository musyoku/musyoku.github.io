...
VPYLMでは文脈木の各レストラン$\boldsymbol u$に、木をルートからたどるときにそこで止まる確率$q_{\boldsymbol u}$があると考えます。

またこれらの停止確率は共通のベータ事前分布から生成されていると仮定します。

$$
	\begin{align}
		q_{\boldsymbol u} \sim {\rm Beta}(\alpha, \beta)
	\end{align}\
$$
...
となっているので、客$dog$がルートのレストランに追加される確率は$q_{root}$、レストラン$lazy$に追加される確率は$(1-q_{root})q_{lazy}$、レストラン$the$に追加される確率は$(1-q_{root})(1-q_{lazy})q_{the}$、・・・のように計算されます。
...
HPYLMと同様に客の配置$\boldsymbol \Theta$も確率変数であり、VPYLMでは$\boldsymbol \Theta$と$\boldsymbol n$の両方をギブスサンプリングによって推定します。

具体的には、単語列$\boldsymbol w$の位置$t$の単語$w_t$の隠れたn-gramオーダー$n_t$を、
...
その状態で式(2)により新たなn-gramオーダー$n_t$をサンプリングします。

また、サンプリングされた深さ$n_t$に単語$w_t$を追加し直すことで$\boldsymbol \Theta$が自動的にサンプリングされます。

$\boldsymbol n_{-t}$は、単語$w_t$を除いた単語列$w_1,w_2,w_{t-1},w_{t+1},w_T$に対応するn-gramオーダー$n_1,n_2,...,n_{t-1},n_{t+1},...,n_T$を表しています。

これはつまり、我々は他の単語が停止した深さ$n_1,n_2,...,n_{t-1},n_{t+1},...,n_T$をすべて知ってる必要があるということです。
